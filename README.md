...
当一个工作区包含多个 CBP 项目时，可以使用 `merge-compile-commands` 命令将它们的 compile_commands.json 合并：

```bash
# 合并两个项目的 compile_commands.json
cbp2clangd merge-compile-commands app.cbp lib.cbp

# 指定输出目录
cbp2clangd merge-compile-commands app.cbp lib.cbp --output-dir C:/workspace
```

合并后：
- 所有项目的编译命令会合并到第一个项目的 compile_commands.json 中
- .clangd 文件会被更新，CompilationDatabase 路径会添加到主 CompileFlags 块中
- 示例输出：
```yaml
CompileFlags:
  CompilationDatabase: C:/workspace/project1/Output/obj
  Add:
    - -xc
    - -target
    ...
